...
## RDB快照
  ...
  RDB 执行频率过低可能导致数据丢失，而频率过高则会增加磁盘写入和子进程创建的开销。通过开启混合持久化功能（`aof-use-rdb-preamble yes`），在 AOF 重写过程中，Redis 会先以 RDB 格式写入数据到 AOF 文件，再将增量命令追加到 AOF 文件中。写入完成后，系统会通知主线程使用新的 RDB 和 AOF 格式的文件替换旧的 AOF 文件。
